...
getData().then((data)=>{
    ...
    document.getElementById('js-filterType').addEventListener('click', (e)=>{
        if (e.target.type == 'radio') filter(baseData);
    });
}).catch((error)=>{
    console.log(error);
});
...
function generateFilterLists(data) {
    ...
    data.map((element)=>{
        if (!yearList.includes(element)) yearList.push(element.year);
        element.genre.map((subElement)=>{
            if (!genreList.includes(subElement)) genreList.push(subElement);
        });
    });
    ...
    function buildFilterDropdown(dropdown, list) {
        ...
        list.map((element)=>{
            ...
            listItem.innerHTML = '<input type="checkbox" id="' + element + '" name="' + element + '" value="' + element + '" class="js-dropdownInput" />' + '<label for="' + element + '">' + element + '</label>';
            dropdown.appendChild(listItem);
            // Runs filter function on click of any of the dropdown filter inputs
            listItem.addEventListener('click', (e)=>{
                if (e.target.classList.contains('js-dropdownInput')) filter(data);
            });
        });
    }
}
...
function buildGenreString(mediaGenres) {
    ...
    mediaGenres.map((element, index)=>{
        if (index == 0) mediaGenreString = mediaGenreString + capitalizeString(element);
        else mediaGenreString = mediaGenreString + ', ' + capitalizeString(element);
    });
...
function gatherMediaProps(mediaItem1) {
    let mediaPoster1 = mediaItem1.poster;
    let mediaTitle1 = mediaItem1.title;
    let mediaYear1 = mediaItem1.year;
    let mediaGenres1 = buildGenreString(mediaItem1.genre);
    buildMediaMarkup(mediaPoster1, mediaTitle1, mediaYear1, mediaGenres1);
    ...
    function buildMediaMarkup(mediaPoster, mediaTitle, mediaYear, mediaGenres) {
        ...
        mediaItem.innerHTML = '<img src="' + mediaPoster + '" class="media-item__img" />' + '<p class="media-item__title">' + mediaTitle + ' (' + mediaYear + ')</p>' + '<p class="media-item__genre">' + mediaGenres + '</p>';
...
function filter(data1) {
    ...
    let searchTerm1 = document.getElementById('js-search').value;
    for(let i1 = 0; i1 < typeFilterList.length; i1++)if (typeFilterList[i1].checked) checkedTypeFilter = typeFilterList[i1].value;
    checkedYearFilterList = getCheckedFilters(yearFilterlist);
    checkedGenreFilterList = getCheckedFilters(genreFilterList);
    data1.map((element)=>{
        if (checkedYearFilterList.length) checkedYearFilterList.map((subElement)=>{
            if (subElement == element.year) newYearData.push(element);
        });
        else newYearData.push(element);
        if (checkedGenreFilterList.length) checkedGenreFilterList.map((subElement)=>{
            let genres = element.genre;
            for(var i = 0; i < genres.length; i++)if (subElement == genres[i]) newGenreData.push(element);
        });
        else newGenreData.push(element);
    ...
    if (newTypeData.length) newData1 = checkArrayForDups(newFilterData, newTypeData);
    else newData1 = newFilterData;
    clearMedia();
    liveData = newData1;
    newData1.map((element)=>{
        gatherMediaProps(element);
    });
    if (searchTerm1.length) search(searchTerm1, liveData);
    ...
    function getCheckedFilters(list) {
        let checkedFilters = [];
        for(let i = 0; i < list.length; i++)if (list[i].checked == true) checkedFilters.push(list[i].value);
    ...
    function search(searchTerm, data) {
        let newData = [];
        clearMedia();
        if (searchTerm.length) data.map((element)=>{
...
},{"../styles/main.scss":"bo7w8"}],"bo7w8":[function() {},{}]},["l4AUa","ebWYT"], "ebWYT", "parcelRequirec6bf")
